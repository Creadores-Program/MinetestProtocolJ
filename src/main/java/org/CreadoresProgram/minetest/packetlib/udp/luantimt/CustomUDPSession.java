...
public abstract class CustomUDPSession extends SimpleChannelInboundHandler<Packet> implements Session{
    ...
    public CustomUDPSession(String host, int port, PacketProtocol protocol){
        this.host = host;
        this.port = port;
        ...
        if(this.channel != null) {
            if(this.compressionThreshold >= 0) {
                if(this.channel.pipeline().get("compression") == null) {
                    this.channel.pipeline().addBefore("codec", "compression", new CustomUDPPacketCompression(this));
                }
        ...
        if(!sendingEvent.isCancelled()) {
            ...
            this.channel.writeAndFlush(toSend).addListener(new ChannelFutureListener() {
                ...
                public void operationComplete(ChannelFuture future) throws Exception {
                    if(future.isSuccess()) {
                        callEvent(new PacketSentEvent(CustomUDPSession.this, toSend));
                    } else {
    ...
    public void disconnect(final String reason, final Throwable cause) {
        ...
        if(this.channel != null && this.channel.isOpen()) {
            ...
            this.channel.flush().close().addListener(new ChannelFutureListener() {
                @Override
                public void operationComplete(ChannelFuture future) throws Exception {
                    callEvent(new DisconnectedEvent(CustomUDPSession.this, reason != null ? reason : "Connection closed.", cause));
                }
            });
        } else {
            ...
            public void run() {
                try {
                    ...
                    while((packet = packets.take()) != null) {
                        callEvent(new PacketReceivedEvent(CustomUDPSession.this, packet));
                    }
